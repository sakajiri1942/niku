...
fname = ('C:/Users/sekis/PycharmProjects/niku/nikuniku-2021-07-15.csv')
    ...
    if '全国' not in l_data:
        ...
        for l_kenbetu_usi in kenbetu_usi:
            ...
            if '(' not in l_kenbetu_usi and len(l_kenbetu_usi) > 2:  # 半角（）
                ...
                price_set_int_sort = sorted(price_set_int)
                # print(price_set_int_sort)
                moto_kazu = len(price_set_int_sort)

                if moto_kazu > 0:
                    matome_en = str(price_set_int_sort[0]) + '～' + str(price_set_int_sort[-1]) + '円'
                    print(matome_en)
